  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      try {
        ...
        const lookAtObj = splineApp.findObjectByName('LookAt') || 
  ...
  const onLoad = (spline: any) => {
    ...
    if (spline) {
      try {
        ...
        const lookAtObj = spline.findObjectByName('LookAt') || 
        ...
        if (allObjects) {
          allObjects.forEach((obj: any) => {
            ...
            if (obj.name.toLowerCase().includes('look') || 
    ...
    <>
      ...
      {debugMode && (
        <div className="fixed top-0 left-0 p-4 bg-black bg-opacity-70 text-white z-50 text-xs font-mono max-w-xs max-h-[80vh] overflow-auto">
          <h3 className="text-green-400 mb-2">Spline Debug (Ctrl+Shift+D to hide)</h3>
          <p>Mouse: x={mousePos.x}, y={mousePos.y}</p>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button 
              ...
              onClick={() => {
                ...
                console.log('Look-at behavior:', eventsRef.current.lookAt ? 'Enabled' : 'Disabled');
              }}
            >
              Toggle Look-at
            </button>
